						...
						if (CmnCnst.URL_PARAM_VALUE_TYPE_PROGRAM.equalsIgnoreCase(type)) {
							if (CmnCnst.URL_PARAM_VALUE_CONTEXT_FORMCYCLE.equalsIgnoreCase(context)) {
								res = FormExpressionFactory.forProgram()
										.parse(code, FormcycleEcContractFactory.INSTANCE, strict)
										.evaluate(new FormcycleExternalContext(DummyWriter.getInstance())).toString();
							}
							else {
								res = FormExpressionFactory.forProgram()
										.parse(code, GenericEcContractFactory.INSTANCE, strict)
										.evaluate(DummyExternalContext.getInstance()).toString();
							}
						}
						else {
							try (final Writer writer = new StringBuilderWriter()) {
								if (CmnCnst.URL_PARAM_VALUE_CONTEXT_FORMCYCLE.equalsIgnoreCase(context)) {
									FormExpressionFactory.forTemplate()
											.parse(code, FormcycleEcContractFactory.INSTANCE, strict)
											.evaluate(new FormcycleExternalContext(writer));
								}
								else {
									FormExpressionFactory.forTemplate()
											.parse(code, GenericEcContractFactory.INSTANCE, strict)
											.evaluate(new WriterOnlyExternalContext(writer));
