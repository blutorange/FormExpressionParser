 ...
 *   <li>AttrAsssigner: Methods for assigning values to properties of various language objects. For example, <code>array[index] = value;</code> or <code>hash.key = value;</code>. The <code>thisContext</code> is the left-hand side. The first string argument is the name of the property assigned to, eg. <code>key</code> for <code>hash.key=9;</code>. The second boolean argument is true when the property is assigned to via dot-notation (eg. <code>hash.key=0;</code>), and false otherwise (eg. <code>hash[key] = 0;</code>). The third argument is the language object which is to be assigned.</li>
