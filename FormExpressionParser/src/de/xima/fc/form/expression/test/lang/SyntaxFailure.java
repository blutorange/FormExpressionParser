...
enum SyntaxFailure implements ITestCase {
	...
	STRICT005(new Cfg("()=>void{string x='';x.<string>matches(#.+#);};").err("Error during parsing at line 1, column 24: No such dot accesor named <string>matches for type string.").err(NoSuchDotAccessorException.class).strict()),
	STRICT006(new Cfg("global scope {array<string>x=['foo','foobar'];}x.<number,number>map((string s)=>{return s.length;});").err("Error during parsing at line 1, column 50: No such dot accesor named <number,number>map for type array<string>.").err(NoSuchDotAccessorException.class).strict()),
	...
	VOID004(new Cfg("global scope{method<void> m;}m().toString;").err("Error during parsing at line 1, column 34: No such dot accesor named toString for type void.").err(NoSuchDotAccessorException.class).strict()),
